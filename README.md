...
- **Threads** - Reply to any message in a threaded conversation
...
- **Real-time Updates** - Livewire polling (configurable intervals)
...
If you haven't already, create the notifications table for database notifications:
...
| Table | Purpose |
|---|---|
| `tc_channels` | Chat channels (public/private) |
| `tc_channel_user` | Channel membership pivot |
| `tc_conversations` | Direct messages (1-on-1 and group) |
...
Conversation    - 1-on-1 and group direct messages
...
The chat UI is composed of 8 Livewire components:

| Component | Purpose | Polling |
|---|---|---|
| `Sidebar` | Channel/DM list, unread badges, create/join | 5s |
| `MessageFeed` | Message list, reactions, edit/delete | 3s |
...
The plugin registers a custom Filament page at `/admin/team-chat` with a full-viewport Slack-like layout. The page uses Filament's authentication middleware while providing a custom chat UI.
...
The package includes a comprehensive test suite. Run tests in your application:
